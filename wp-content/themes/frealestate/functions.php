...
if ( ! function_exists( 'frealestate_setup' ) ) :
	...
	function frealestate_setup() {
		...
		add_image_size( 'frealestate-thumbnail-avatar', 100, 100, true );
...
if ( ! function_exists( 'frealestate_customize_register' ) ) :
	...
	function frealestate_customize_register( $wp_customize ) {
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'frealestate_slider_display',
								array(
									'label'          => __( 'Display Slider', 'frealestate' ),
		...
		for ($i = 1; $i <= 3; ++$i) {
			...
			$wp_customize->add_setting(
				$slideContentId,
				array(
					'default'           => __( '<h2>This is Default Slide Title</h2><p>You can completely customize Slide Background Image, Title, Text, Link URL and Text.</p><a class="btn" title="Read more" href="#">Read more</a>', 'frealestate' ),
		...
		<div class="slider">
			...
			<?php
				...
				for ( $i = 1; $i <= 3; ++$i ) {
					...
					$defaultSlideContent = __( '<h2>This is Default Slide Title</h2><p>You can completely customize Slide Background Image, Title, Text, Link URL and Text.</p><a class="btn" title="Read more" href="#">Read more</a>', 'frealestate' );
						
					$defaultSlideImage = get_template_directory_uri().'/img/' . $i .'.jpg';

					$slideContent = get_theme_mod( 'frealestate_slide'.$i.'_content', html_entity_decode( $defaultSlideContent ) );
